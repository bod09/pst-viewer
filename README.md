...
- **Fast search** across all open mailboxes at once (each result says which mailbox it came from): subjects, senders, recipients, body text, attachment names, and text inside images. A filter panel opens when you click the search box, so nothing has to be memorised, and searches can be narrowed to one **mailbox** or **folder** when you only care about part of what is open. The finished search index is kept on your device (like the OCR cache, cleared 7 days after a mailbox was last opened), so reopening the same file is searchable straight away instead of being scanned again. Words are typo-tolerant (fuzzy); numbers and reference codes are matched **exactly**, so an ID search stays precise. Matches are highlighted in the open email (the text, and the matching picture) and it scrolls to the first hit.
- **OCR** (automatic, can be turned off): text inside images is read in the background and made searchable, covering both image attachments and pictures embedded in the email body. A toggle in **Settings** (the gear button) disables it entirely, including its cache; the choice is remembered. Images are sharpened first to read small text more reliably, and results are cached on your device for faster re-opens (cleared 7 days after a mailbox was last opened). Engine and model are bundled for full offline use.
- **Settings** (the gear button, top right): read text in images on or off, load images from the internet on or off, show or hide empty folders, the theme and accent colours, and a button to clear any pictures already fetched from the internet.
- **Keyboard**: the folder tree, message list and search results can all be driven from the keyboard (arrow keys to move, Enter to open, Left and Right to collapse and expand a folder), and dialogs keep focus inside them and hand it back when they close.
...
There is no server. When you open a file, the browser reads it **directly from your disk** (in small slices, so even multi-gigabyte mailboxes work) and all parsing, rendering, search, OCR, and PDF export happen on your device. Your mailbox is never uploaded. Like a normal mail client, an email that references **remote images** will fetch those from the sender's servers when you view it (invisible tracking pixels are stripped, but a visible remote image can still tell the sender you opened it). Each remote image is fetched only once and then cached locally in your browser, so re-viewing it does not ping the sender again. Those saved pictures are a record of what the mail linked to, so Settings shows how many are held and can clear them; switching the setting off clears them too. Turning off **Load images from the internet** in Settings stops this completely: remote images, stylesheets and fonts are all removed and the message is rendered under a policy that forbids reaching another server, so opening it cannot tell the sender anything. Messages then show only what is stored inside the file. Apart from that, the only network use is loading the app itself.
...
- Search becomes available for a mailbox once its background indexing finishes (a progress indicator is shown). If the browser runs short of memory part-way through a very large mailbox, a folder can be lost from that pass: the sidebar then says search may be incomplete for it, and nothing from that pass is saved, so opening the file again indexes it afresh.
